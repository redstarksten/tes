...
class prankcall{
	...
	public function run(){
		while(true){
			echo "Lebih dari 1 kali (y/n) = ";
			$loop = $this->get();
			if($loop=="y" OR $loop=="n"){
				break;
			}else{
				echo "Ketik y jika ingin lebih dari 1 kali spam, ketik n jika hanya 1 kali";
				continue;
			}
		}
		if($loop=="y"){
			echo "Total Spam yang diinginkan = ";
			$many = $this->get();
			$this->loop($many);
		}else{
			$this->ekse();
